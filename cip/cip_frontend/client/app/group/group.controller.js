...
angular.module('cipApp')
  .controller('groupCtrl', function ($rootScope, $scope, $state, $translate, $stateParams, $location, groupFactory, crowdFactory) {
    ...
    $scope.init_create = function() {
      ...
      $scope.updateCrowdSelectedList = function() {
        $scope.crowd.list.forEach(function(d) {

          d.__selected = false;

          $scope.groupDetail.selector.some(function(t) {
            if (t._id == d._id) {
              d.__selected = true;
              return true;
            }
          ...
          if ($stateParams.groupID !== undefined) {
            ...
            groupFactory.createGroup($scope.groupDetail, function (data) {
                if (data.success) {
                    ...
                    $location.path('/crowd/' + data._id + '/group_user');
                } else {
    ...
    $scope.init_user = function() {
      ...
      groupFactory
      .getGroup(groupID, function(data) {
        console.log(data);
        if (data.success) {
      ...
      groupFactory
      .getGroupSample(groupID, 10, 'group', function(data) {
        if (data.success) {
