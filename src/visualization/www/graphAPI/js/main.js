  ...
  var filteringStatus = {node:{type:true, id:false}, 
                          edge:{type:true, id:false}}; // node:{type:boolean, attr1:boolean, attr2:boolean ...}
    ...
    */
    ...
    for (var i=0; i < elements.length; i++){
      ...
      if (elements[i]["dropdown"]) {
        s += '&nbsp;&nbsp;<select class="form-control" name='+ elements[i]["dropdown"]["name"] +'>'

        elements[i]["dropdown"].value.forEach(function(type_name, i) {
  ...
  this.layoutChanged = function(newLayout) {
    ...
    $('#tableBox').height($('#left_side').height() - $('#layoutport').height() - 20)
