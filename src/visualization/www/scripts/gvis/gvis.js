...
var gsqlv = function(x) {
  ...
  gsqlv.startLayout = function() {
    // START Hack layout algorithm for shandong project.
    if (window.pages_obj[page_index].tabname == "Customer 360" || window.pages_obj[page_index].tabname == "Realtime Customer 360") {
      return gsqlv.startLayout_forShandong_c360();
      //return ;
    }
    else if (window.pages_obj[page_index].tabname == "Phone Fraud") {
      return gsqlv.startLayout_forShandong_fraud();
    }
    // End Hack layout algorithm for shandong project.
  ...
  gsqlv.startLayout_forShandong_c360 = function() {
    layouts["force"].stop();

    var areas = ["complaint_area", "recommandation_area", "potential_area", "enterprise_area"]
    ...
    console.log("Start Layout For Shandong Specifically.")
    ...
    if (layout == "force") {
      ...
      // circle layout only supports the graph with 4 or more nodes.
      // circle layout is generated by a tree layout algorith + rotation function.
      if (data.nodes.length <= 2) {
        if (typeof data.nodes[0] != 'undefined') {
          data.nodes[0].x = window.v_width / 2 + 80;
      ...
      root = {"children":[], "node":node_links[rootNode], "_depth":1}
      ...
      for (area in areas) {
        root.children.push({node:{}, children:[], _depth:1, name:areas[area]});
      }

      tempChildren.forEach(function(n) {
        switch (n.node.node.type) {
          case "warning":
          case "level":
            //root.children[0].children.push(n);
            // Fix the first area as the complaint node "warning" or "level"
            root.children[0] = n; 
            if (n.children.length == 0) {
              n.children.push({"children":[], "node":{}, "_depth":2});
            }
            break;
          case "app":
            root.children[1].children.push(n);

            break;
          case "phone_no":
            root.children[2].children.push(n);
            
            break;
          default:
            root.children[3].children.push(n);
            break;
        }
      })

      root.children[3].children.push({node:{}, children:[], _depth:2});

      layouts["circle"] = d3.layout.tree()
          .size([separation_x, separation_y*tempMaxDepth*2])
          .separation(function(a, b){
            return (a.parent == b.parent ? 1 : 4) / Math.pow(a.depth, 1.5);
          })

      circledNodes = layouts["circle"].nodes(root);

      // decrese the gap for depth==1 nodes.
      // tempRootX = 0;
      // tempRootY = 0;
      // circledNodes.forEach(function(n){
      //   if (n._depth == 1) {
      ...
      circledNodes.forEach(function(n) {
        //"transform" : "rotate(n.x - 90);translate(n.y)"
        if (typeof n.node.node == 'undefined') {
          return;
        }
      ...
      layouts["tree"] = d3.layout.tree()
                ...
                .separation(function(a, b){
                  //return (a.parent == b.parent ? 1 : 2) / a.depth;
                  return (a.parent == b.parent ? 1 : 4) / Math.pow(a.depth, 1);
                })
      ...
      var setTreeData = function (_this, depth) {
        ...
        _this.node.links.forEach(function(node, i) {
          if (!node.hierarchied) {
            ...
            tempTreeNode = {"children":[], "node":node_links[nodeKey], "_depth":depth}
            _this.children.push(tempTreeNode)
            tempNumber += 1;
          }
        })
      ...
      for (area in areas) {
        root.children.push({node:{}, children:[], name:areas[area], _depth:1});
      }
      ...
      //root.children[3].children.push({node:{}, children:[]});
  ...
  gsqlv.startLayout_forShandong_fraud = function() {
    ...
    if (layout == "force") {
      ...
      var setHierarchicalPosition = function (depth, links) {
        ...
        links.forEach(function(node, i) {
          if (!node.hierarchied) {
            ...
            node.x = (i-localSpace) * separation_x + node_links[rootNode].node.x;
            node.y = separation_y * depth + node_links[rootNode].node.y;
            x_position = (i-localSpace) * separation_x;
          }
          else {
            localSpace += 1;
          }
      ...
      node_links[rootNode].node.x = setting.width / 2 + max_x_position / 2;  
      node_links[targetNode].node.x = setting.width / 2 + max_x_position / 2;  
      ...
      var targetIndex = -1;
      root.children.forEach(function(n, i) {
        var node = n.node.node;
        if (targetNode == node.type+"&"+node.id) {
          //console.log(" Target Node in Children of Root.")
          targetIndex = i
        }
      })

      var checkNoShortestPath = false;
      data.links.some(function(l) {
        if (l.type == "no_shortest_path") {
          checkNoShortestPath = true;
          return checkNoShortestPath;
        }
        else {
          return checkNoShortestPath;
        }
      ...
      /** Hack layout for Phone Fraud **/
      if (targetIndex == -1 || !checkNoShortestPath) {
        // Do not change the layout.
        separation_y = separation_y / 3;
      }
      else {
        var tempChain3 = {node:{node:{}}, children:[root.children[targetIndex]], _depth:3}
        var tempChain2 = {node:{node:{}}, children:[tempChain3], _depth:2}
        var tempChain1 = {node:{node:{}}, children:[tempChain2], _depth:1}

        root.children.splice(targetIndex, 1);
        root.children.unshift(tempChain1);
      }
      

      /** End Hack layout for Phone Fraud **/
  ...
  gsqlv.outputTableforSelectedNodes = function() {
    ...
    */
    ...
    for (var node in selectedNodes) {
      ...
      // Do something for Shandong project specificly.
      if (node.type == "phone_no" || node.type == "phone_no_on_path") {
        ...
        var priorAttrs = ["term_type","fact_fee", "data_fee", "gprs_flow"];

        for (var i in priorAttrs) {
          var key = priorAttrs[i]

          if (node.attr.hasOwnProperty(key)) {
            nodeItem += '<tr name="attribute_name" onclick="onClick_table(this,\'node\')"><td style="line-height:1.0;padding:4px">'
        ...
        var skipAttrs = ["arpu", "cal_arpu", "local_cal_arpu", "long_cal_arpu", "roam_cal_arpu", 
                  "gsm_flow", "td_flow", "lte_flow"]
        ...
        var flowData=[
          {label:"2G", color:"#3366CC", value:node.attr["gsm_flow"]},
          {label:"3G", color:"#DC3912", value:node.attr["td_flow"]},
          {label:"4G", color:"#FF9900", value:node.attr["lte_flow"]}
        ];
        ...
        svg.append("g").attr("id","flowDonut");

        Donut3D.draw("flowDonut", flowData, 110, 70, 105, 65, 20, 0.4);
        var tempFlowChart = d3.select('#flow_chart')[0][0].innerHTML//.outerHTML;
        d3.select('#flow_chart').remove();

        tempFlowChart = '<svg width="'+chart_width+'" height="'+chart_height+'">' + tempFlowChart + '</svg>'

        //nodeItem += '<tr><td colspan="2">'+example_chart2+'</td></tr>'
        nodeItem += '<tr><td>'+toLanguage('flow')+'</td><td>'+tempFlowChart+'</td></tr>'
        ...
        var flowData=[
          {label:"local_cal_arpu", color:"#DC3912", value:node.attr["local_cal_arpu"]},
          {label:"long_cal_arpu", color:"#FF9900", value:node.attr["long_cal_arpu"]},
          {label:"roam_cal_arpu", color:"#3366CC", value:node.attr["roam_cal_arpu"]}
        ];
        ...
        var width = chart_width - margins.left - margins.right;
        var height = chart_height + margins.top + margins.bottom;

        xMax = d3.sum(flowData, function (d) {
        ...
        groups = svg.selectAll('.rect')
            .data(flowData, function (d) {
                return d.label;
            })
            .enter()
        //     .append('g');
        ...
        // groups 
            ...
            .attr('x', function (d) {
              var temp = width_position
              width_position += xScale(parseFloat(d.value))
              return temp;
            })
            .attr('y', function (d, i) {
        ...
        svg.selectAll('.text')
            ...
            .attr('x', function (d) {
              var temp = width_position
                width_position += xScale(parseFloat(d.value))
                return temp + xScale(parseFloat(d.value)) / 2.0;
            })
            .attr('y', function (d, i) {
                return chart_height/2 + chart_height/4;
            })
            .attr('text-anchor', "middle")
        ...
        var legends = svg.selectAll('.legend')
           .data(flowData, function (d) {
            ...
            .each(function(d){
              ...
              .attr('fill', function(d){return d3.rgb(d.color).darker(3)})
              .attr('x', function(d){
                return temp + chart_height/8 + 4;
              })
              .attr('y', function (d, i) {
                  return chart_height/4;
              })
              .attr('text-anchor', "start")
              .attr('font-size', '10px')
              .text(function(d){return ":"+toLanguage(d.label)})
            })
...
// });
        ...
        //nodeItem += '<tr><td colspan="2">'+example_chart2+'</td></tr>'
        nodeItem += '<tr><td>'+toLanguage('cal_arpu')+'</td><td>'+tempFlowChart+'</td></tr>'

        /*************** End Chart 2 ***************/

        for (var key in node.attr) {
          if (skipAttrs.indexOf(key) > -1 ) continue;
          if (priorAttrs.indexOf(key) > -1 ) continue;
  ...
  */
  ...
  function refineDataName(newData) {
    ...
    if (page_index == 2) { // Recommandation.
      // hack recommandation result.
      ...
      var tempNode = {"type":"","id":"","attr":""};
      tempNode.type = ttype
      tempNode.id = tid

      $.ajax({
           url:'/engine/graph/vertices/'+ttype+'/'+tid,
           success: function(message) {
                      tempNode.attr = message.results[0].attributes;
                    },
           async:   false
      });
      ...
      newData.forEach(function(n) {
        ...
        tempNode.type = "app"
        tempNode.id = n.id
        tempNode.attr = {'score':n.score, 'name':n.name};
      ...
      // Add node done.
      // Deduplicate and Create nodes list.
      var tempResult = {};
      ...
      // Add edge for phone to app.
      // Add edge for app to reason.
      var checkReason = {}
      newData.forEach(function(e) {
        ...
        tempEdge.source.type = ttype
        tempEdge.source.id = tid
        tempEdge.target.type = "app"
        tempEdge.target.id = e.id
        ...
        // Add edge for app to reason.
        tempEdge = {"source":{"type":"","id":""},"target":{"type":"","id":""},"attr":""};
        tempEdge.source.type = "app"
        tempEdge.source.id = e.id
        tempEdge.target.type = "reason"
        tempEdge.target.id = e.query_name

        checkReason[e.query_name] = true;
      ...
      for (var reason in checkReason) {


        // add location node.
        if (reason == "location") {
          tempNode = {"type":"","id":"","attr":""};
          tempNode.type = "reason"
          tempNode.id = "location"
          tempNode.attr = {'reason':'Many people in the same location are using this APP.'}; //您周围的用户正在使用的app.'}//
          //tempNode.other = clone(n.other)
        ...
        if (reason == "contact") {
          ...
          tempNode.type = "reason"
          tempNode.id = "contact"
          tempNode.attr = {'reason':'Some your friends are using this APP.'}; // 您的朋友正在使用的app.'}//
          //tempNode.other = clone(n.other)
        ...
        if (reason == "CF") {
          ...
          tempNode.type = "reason"
          tempNode.id = "CF"
          tempNode.attr = {'reason':'Collaborative filtering recommands this APP to you.'}; //与您兴趣相同的用户正在使用的app.'}//
          //tempNode.other = clone(n.other)
    ...
    else if (page_index == 3 || page_index == 4 || page_index == 5) {
      // the result should be as nodes and edges.
      var result = {"nodes":[], "edges":[]};
      var arrResult = []
    ...
    else if (page_index == 0 || page_index == 7) {
      // the result should be as nodes and edges.
      // 1. complaint default
      var result = {"nodes":[], "edges":[]};
      var arrResult = []
      ...
      */
      ...
      // 2. Recommandation, index : 2
      // add app node
      var recommandationUDFIndex = 2;
      submit_URL = window.pages_obj[recommandationUDFIndex].events.submit.URL_head;
      payload = window.pages_obj[recommandationUDFIndex].events.submit.URL_attrs.payload[0]
      ...
      for (var attr in payload.attr) {
        ...
        if (attr.usage == "input") {
          ...
          tempInput = window.pages_obj[recommandationUDFIndex].attributes[attr.name];
          submit_payload = submit_payload.replace(new RegExp("{{"+attr.attr+"}}", "g"), tempInput)
        }
        else if (attr.usage == "select") {
      ...
      newData.forEach(function(n) {
        ...
        tempNode.type = "app"
        tempNode.id = n.id
          tempNode.attr = {'score':n.score, 'name':n.name == "" ? "app Name" : n.name};
        tempNode.other = clone(n.other)
      ...
      for (var reason in checkReason) {
        ...
        if (reason == "location") {
          ...
          tempNode.type = "reason"
          tempNode.id = "location"
          tempNode.attr = {'reason':'您周围的用户正在使用的app..'};
          //tempNode.other = clone(n.other)
        ...
        if (reason == "contact") {
          ...
          tempNode.type = "reason"
          tempNode.id = "contact"
          tempNode.attr = {'reason':'您的朋友正在使用的app.'};
          //tempNode.other = clone(n.other)
        ...
        if (reason == "CF") {
          ...
          tempNode.type = "reason"
          tempNode.id = "CF"
          tempNode.attr = {'reason':'与您兴趣相同的用户正在使用的app.'};
          //tempNode.other = clone(n.other)
      ...
      //3 Group User.
      var groupUserUDFIndex = 4;
      temp_event = window.pages_obj[groupUserUDFIndex].events.submit 
      submit_URL = temp_event.URL_head;
      submit_payload = {};
      ...
      */
      ...
      for (var attr in URL_attrs.maps) {
        ...
        if (attr.usage == "input") {
          ...
          submit_URL += name+ "=" + window.pages_obj[groupUserUDFIndex].attributes[attr.name] + "&";
        }
      }
      ...
      // Deduplicate and Create Edges list.
      var tempResult = {};
      result.edges.forEach(function(n, i){
        tempResult[n.source.id + '-'+ n.source.type+'_'+n.target.id+'-'+n.target.type] = n; 
      })

      result.edges = [];

      for (var key in tempResult) {
        result.edges.push(tempResult[key]);
      }
    }
    else {
      // the result should be as nodes and edges.
    ...
    //Refined the Visualization Result for unuseful node of complaint.
    // 1. remove bypass_complaint node and edge "109" if bypass_complaint_count == 0 
    var isCountZero = false;

    for (var i=result.nodes.length-1; i>=0;--i) {
      n = result.nodes[i];
      if (n.type == 'bypass_complaint' && n.id == 'bypass_complaint' && n.attr.bypass_complaint_count == "0") {
    ...
    if (isCountZero) {
      for (var i=result.edges.length-1; i>=0;--i) {
        l = result.edges[i];
        if ((l.source.type == "bypass_complaint" && l.target.type == "warning") || (l.target.type == "bypass_complaint" && l.source.type == "warning")) {
          result.edges.splice(i, 1);
        }
      }
    }
    ...
    // 1. Remove the warning node, if iswarning == false.
    // a. check count of location, and friend. if true remove it.
    // b. if both false, remove warning node.
    // c. Remove warning node, if it has complaint_typeA, or if it does not have other complaint_type.

    /*
    var tempRemoveLocation = false;
    var tempRemoveFriend = false;
    var tempRemoveWarning = false;
    var tempHasTypeA = false;
    var tempHasOtherType = false;

    var tempRemoveTypeArray = []

    for (var i=result.nodes.length-1; i>=0;--i) {
      n = result.nodes[i];
      if (n.type == 'location' && n.id == 'location' && n.attr.count == "0") {
          result.nodes.splice(i, 1);
          tempRemoveLocation = true;
      }
      else if (n.type == 'friend' && n.id == 'friend' && n.attr.count == "0") {
          result.nodes.splice(i, 1);
          tempRemoveFriend = true;
      }
      else if (n.type == 'warning' && n.id == 'warning' && n.attr.is_warning == "No") {
          result.nodes.splice(i, 1);
          tempRemoveWarning = true;
      }
      else if (n.id == 'Complaint_Type_A') {
          tempHasTypeA = true;
      }
      else if (n.id == 'Complaint_Type_B' || n.id == 'Complaint_Type_C' || n.id == 'Complaint_Type_D') {
          tempHasOtherType = true;
      }
    }

    for (var i=result.nodes.length-1; i>=0;--i) {
      n = result.nodes[i];
      if (n.type == 'location' && n.id == 'location' && (tempHasTypeA || !tempHasOtherType)) {
          result.nodes.splice(i, 1);
          tempRemoveLocation = true;
      }
      else if (n.type == 'friend' && n.id == 'friend' && (tempHasTypeA || !tempHasOtherType)) {
          result.nodes.splice(i, 1);
          tempRemoveFriend = true;
      }
      else if (n.type == 'warning' && n.id == 'warning' && (tempHasTypeA || !tempHasOtherType)) {
          result.nodes.splice(i, 1);
          tempRemoveWarning = true;
      }
    }
    ...
    for (var i=result.edges.length-1; i>=0;--i) {
      l = result.edges[i];
      if (tempRemoveLocation) {
        if (l.source.type == "location" || l.target.type == "location") {
          result.edges.splice(i, 1);
        }
      }

      if (tempRemoveFriend) {
        if (l.source.type == "friend" || l.target.type == "friend") {
          result.edges.splice(i, 1);
        }
      }

      if (tempRemoveWarning) {
        if (l.target.type == "phone_no" && l.source.type == "warning") {
          result.edges.splice(i, 1);
        }
      }
    }
