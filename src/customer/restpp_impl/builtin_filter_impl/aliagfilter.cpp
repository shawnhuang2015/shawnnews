#ifndef ALIAGFILTER_CPP
#define ALIAGFILTER_CPP

#include "ali_cm_graph_info.hpp"
#include "ali_load_raw_account_file.hpp"
#include "ali_load_realtime_account_file.hpp"
#include "../parse.hpp"



bool AliAgFilter(RequestInfo *request_info,
                              ...
                              Handler *handler)
{
  ...
  if(tag == "realtime_account"){
    ALI_CM_GRAPH_LOADING::AliRealtimeAccountFileLoader alievent_loader(&handler->csv_post_reader,
                                                           &handler->csv_post_writer,
                                                           &handler->delta_buffer_writer);
    alievent_loader.startLoad(processing_result->error_message);
  }
  else if(tag == "raw_account"){
    ALI_CM_GRAPH_LOADING::AliRawAccountFileLoader alievent_loader(&handler->csv_post_reader,
                                                           &handler->csv_post_writer,
                                                           &handler->delta_buffer_writer);
    alievent_loader.startLoad(processing_result->error_message);
  }
