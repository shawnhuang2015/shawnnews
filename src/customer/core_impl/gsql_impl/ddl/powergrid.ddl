...
create vertex BUS(primary_id BusId string, exId string, BusName string, NominalV float, PuV real, VAngle real, LoadP float, LoadQ float, GenP float, GenQ float, Switched float, GShunt float, BShunt float, Vr real, Vi real, Latitude float, Longitude float) with stats="OUTDEGREE_BY_EDGETYPE"

create undirected edge BRANCH(from BUS, to BUS, CID string, R real, X real, hB real, Ratio real, Shift real, Type real, MVALimitA real, MVALimitB real, MVALimitC real)
