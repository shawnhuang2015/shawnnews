...
var gsqlv = function(x) {
  ...
  gsqlv.runPreDefinition = function(nodes, links) {
    ...
    try {
      ...
      tempNode = node_links[targetNode].node;
        ...
        if (c.selectionType == "nodes") {
          nodes.forEach(function(n){
            if (gsqlv.checkConditionForNode(n, c.selection)) {
              n[c.type] = c.color;//color(c.color)
            }
          })
        }
          ...
          links.forEach(function(l){
            if (gsqlv.checkConditionForEdge(l, c.selection)) {
              l[c.type] = c.color;//color(c.color)
            }
          })
        }
        else {
  ...
  */
  ...
  function refineDataName(newData) {
    ...
    newData.paths.forEach(function(p) {
      p.path.forEach(function(n, i) {
        if (i % 2 == 0) {
          if (n in nodes) {
      ...
      p.path.forEach(function(n, i) {
        if (i > p.path.length-3) {
          return;
        }
        else if (i % 2 == 0){
          var tempEdge = {src:n, tgt:p.path[i+2], attr:{Calls:p.path[i+1]}}
          edges.push(tempEdge)
        }
      })
    })
    ...
    for (var node in nodes) {
      ...
      tempNode.type = n.type?n.type:0;
      tempNode.id = n
      tempNode.attr = n.attr?n.attr:{};
    ...
    edges.forEach(function(e) {
      ...
      tempEdge.source.type = e.src.type?e.src.type:0;
      tempEdge.source.id = e.src;
      tempEdge.target.type = e.tgt.type?e.tgt.type:0;
      tempEdge.target.id = e.tgt
